...
static int decode_packet(int *got_frame, int cached)
{
    ...
    *got_frame = 0;
    ...
    if (*got_frame) {
        ...
        {
            audio_dst_file = fopen(tempfilename, "ab");
        }
...
void encodebody()
{
    ...
    data_size = fread(inbuf, 1, 4608, fin);
    ...
    while (data_size > 0) {
        ...
        for (j = 0; j < (c->frame_size) - 1; j++){
            int16_t val = inbuf[pos];
        ...
        if(data_size < 4608)
        {
            ...
            fin = fopen(tempfilename, "wb");
            fwrite(inbuf, 1, data_size, fin);
